...
Set the workflow's default `run` shell to `pwsh`. Run the fixed manifest capture and exporter below paths built with `Join-Path $env:RUNNER_TEMP`. Validate the CLI's two exact summary lines before appending `has_candidates`, `candidate_count` and `release_tag` through `$env:GITHUB_OUTPUT`. Immediately before attestation, enumerate candidate files with `Get-ChildItem -LiteralPath $candidateDir -Filter '*.json' -File`, sort them by name, and require their actual count to equal `candidate_count` and be within 1 to 1,000. Give the attestation step id `attest`, then require a non-empty `${{ steps.attest.outputs.attestation-id }}` in the next normally success-gated candidate step before draft creation. Do not use `always()`. Create a draft release targeting `$env:GITHUB_SHA`, then partition the sorted full paths into deterministic contiguous batches of at most 64 and pass one batch at a time to `gh release upload` without `--clobber`; do not rely on shell glob expansion. Keep create and every upload in the same token-bearing step. Immediately after create and every upload batch, check `if ($LASTEXITCODE -ne 0) { throw 'GitHub CLI release operation failed.' }` before continuing. Publish with `--latest=false` in the second token-bearing step only after the complete batch loop succeeds, and perform the same immediate check after publication. This must stop a failed create before upload and any failed upload batch before later batches or publication, so a pre-existing tag, draft or release remains untouched. A failed batch may leave a private draft containing earlier successful batches; the existing private failed-draft handling applies. A later release-command failure can still leave an orphan public Sigstore attestation and transparency-log entry without a public Release; it does not authorise a retry or release mutation. Derive deterministic aggregate notes from the validated timestamp/count and state that every immutable asset contains its own attribution.
