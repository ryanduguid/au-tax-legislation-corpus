...
Assert exact `workflow_dispatch:`, no inputs, repository/ref guard, `windows-latest`, `defaults.run.shell: pwsh`, 120-minute timeout, concurrency `publish-live-evidence-v2`, `cancel-in-progress: false`, exact permissions, fixed manifest, `uv==0.12.0`, 1,000-candidate bound, zero-candidate gates, one `actions/attest` v4.2.2 invocation, exact ref before draft before upload before publish, `latest: false`, full action SHAs and no reusable/third-party release action. Assert runner-private paths use `Join-Path` from `$env:RUNNER_TEMP`, outputs append through `$env:GITHUB_OUTPUT`, assets are enumerated and sorted with `Get-ChildItem`, and their actual count equals `candidate_count` and is within 1 to 1,000 immediately before attestation. Require the attest step id `attest`, a non-empty `steps.attest.outputs.attestation-id` check before ref creation, normal success gating with no `always()`, and `GH_TOKEN` only on the two release-transaction steps. Require exact `gh api` string-field creation of `refs/tags/$releaseTag` at `$env:GITHUB_SHA`, a pre-existing-ref failure, `gh release create --verify-tag` with no `--target`, deterministic contiguous upload batches of at most 64 in the same first token-bearing step, no `--clobber`, publication only after the batch loop, and an immediate `$LASTEXITCODE` failure check after every `& gh` invocation.
...
Set the workflow's default `run` shell to `pwsh`. Run the fixed manifest capture and exporter below paths built with `Join-Path $env:RUNNER_TEMP`. Validate the CLI's two exact summary lines before appending `has_candidates`, `candidate_count` and `release_tag` through `$env:GITHUB_OUTPUT`. Immediately before attestation, enumerate candidate files with `Get-ChildItem -LiteralPath $candidateDir -Filter '*.json' -File`, sort them by name, and require their actual count to equal `candidate_count` and be within 1 to 1,000. Give the attestation step id `attest`, then require a non-empty `${{ steps.attest.outputs.attestation-id }}` in the next normally success-gated candidate step before any release transaction. Do not use `always()`. In the first token-bearing step, atomically create `refs/tags/$releaseTag` at `$env:GITHUB_SHA` with `& gh api --method POST repos/ryanduguid/au-tax-legislation-corpus/git/refs --raw-field "ref=refs/tags/$releaseTag" --raw-field "sha=$env:GITHUB_SHA"`; a pre-existing ref must fail. Immediately check `$LASTEXITCODE`, then create the draft with `& gh release create --verify-tag` and no `--target`. Partition the sorted full paths into deterministic contiguous batches of at most 64 and pass one batch at a time to `& gh release upload` without `--clobber`; do not rely on shell glob expansion. Keep ref creation, release creation and every upload in the same token-bearing step. After every `& gh` invocation, check `if ($LASTEXITCODE -ne 0) { throw 'GitHub CLI release operation failed.' }` before continuing. Publish with `& gh release edit --latest=false` in the second token-bearing step only after the complete batch loop succeeds, and perform the same immediate check after publication. A failed ref creation changes nothing and stops. After ref creation succeeds, a later create, upload or publish failure can leave an orphan public tag, in addition to the public Sigstore attestation and any private failed draft containing earlier successful batches. The workflow never deletes, moves, repairs or reuses any of them; a future run with the same identity fails until explicit operator handling. Derive deterministic aggregate notes from the validated timestamp/count and state that every immutable asset contains its own attribution.
