...
| Version | Latest known version of each document; the card gives the collection date as 10 March 2025 | Legislation: current compilation at build time, with `compilation_number`, `compilation_date` and `version_is_current` on every row. Rulings: `fetched_on` only; no compilation status |
